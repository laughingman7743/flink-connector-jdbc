...
class SpannerCatalogTestBase implements JdbcITCaseBase, SpannerTestBase {
    ...
    protected static final TableRow TABLE_SIMPLE_WITH_SCHEMA = createTableWithSchema("test_simple_with_schema");
    protected static final TableRow TABLE_ALL_TYPES = createTableAllTypes("test_all_types");
    protected static final TableRow TABLE_ALL_TYPES_SINK = createTableAllTypesSink("test_all_types_sink");
    protected static final TableRow TABLE_ARRAY_TYPES = createTableArrayTypes("test_array_types");
    protected static final TableRow TABLE_GROUPED_BY_SINK = createTableGroupedBy("test_grouped_by_sink");
    ...
    protected static TableRow createTableAllTypesSink(String name) {
        // The JSON type is not supported, so it is excluded.
        return spannerTableRow(
                name,
                pkField("id", dbType("STRING(36) NOT NULL"), DataTypes.STRING().notNull()),
                field("col_bool", dbType("BOOL"), DataTypes.BOOLEAN()),
    ...
    protected static TableRow createTableArrayTypes(String name) {
        return spannerTableRow(
                ...
                field("col_arr_bytes", dbType("ARRAY<BYTES(MAX)>"), DataTypes.ARRAY(DataTypes.BYTES())),
                ...
                field("col_arr_numeric", dbType("ARRAY<NUMERIC>"), DataTypes.ARRAY(DataTypes.DECIMAL(38, 9))),
                // INVALID_ARGUMENT: Value has type ARRAY<FLOAT64> which cannot be inserted into column col_arr_float32, which has type ARRAY<FLOAT>
                // field("col_arr_float32", dbType("ARRAY<FLOAT32>"), DataTypes.ARRAY(DataTypes.FLOAT())),
                field("col_arr_float64", dbType("ARRAY<FLOAT64>"), DataTypes.ARRAY(DataTypes.DOUBLE())),
                field("col_arr_string", dbType("ARRAY<STRING(MAX)>"), DataTypes.ARRAY(DataTypes.STRING())),
                field("col_arr_timestamp", dbType("ARRAY<TIMESTAMP>"), DataTypes.ARRAY(DataTypes.TIMESTAMP())));
    }
    ...
    static void beforeAll() throws SQLException {
        ...
        Properties props = JdbcConnectionOptions.getBriefAuthProperties(
